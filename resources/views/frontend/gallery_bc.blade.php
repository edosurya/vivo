						...
						<div class="item">
							<div class="work">
								<div class="img d-flex align-items-center justify-content-center rounded" style="background-image: url({{ asset($img['path'])}});">
									<a class=" gallery-item icon d-flex align-items-center justify-content-center" href="{{ asset($img['path'])}}">
										<img src="{{ asset($img['path'])}}" class="img-fluid gallery" alt="{{ asset($img['title'])}}" style="display:none" width="1">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const imgs = galleryGrid.querySelectorAll("img.gallery");
  console.log(imgs);
  ...
  function createIndicators (img) {
    ...
    const parentCol = img.closest('.item');
  ...
  for (const link of links) {
    link.addEventListener("click", function (e) {
      ...
      if (lightboxCarousel) {
        const parentCol = link.closest('.item');
